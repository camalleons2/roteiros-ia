...
// ========== ROTA DE RESET FORÇADO (USE UMA VEZ E DEPOIS REMOVA) ==========
app.get('/api/reset-db-force', (req, res) => {
  const fs = require('fs');
  const dbPath = path.join(__dirname, 'database.sqlite');
  ...
  try {
    ...
    // Renomeia o banco antigo (faz um backup automático)
    if (fs.existsSync(dbPath)) {
      fs.renameSync(dbPath, backupPath);
      console.log(`📦 Banco antigo movido para: ${backupPath}`);
    }

    // Cria um banco NOVO diretamente no lugar do antigo
    const novoDb = new sqlite3.Database(dbPath);
    novoDb.serialize(() => {
      // Tabela usuarios
      novoDb.run(`CREATE TABLE usuarios (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        nome TEXT UNIQUE NOT NULL,
        senha TEXT NOT NULL,
        email TEXT UNIQUE,
      ...
      // Tabela creditos
      novoDb.run(`CREATE TABLE creditos (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        usuario_id INTEGER NOT NULL,
        data TEXT NOT NULL,
        usado INTEGER DEFAULT 0,
      ...
      // Tabela codigos
      novoDb.run(`CREATE TABLE codigos (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        codigo TEXT UNIQUE NOT NULL,
        usado INTEGER DEFAULT 0,
        criado_em DATETIME DEFAULT CURRENT_TIMESTAMP,
    ...
    res.send(`
      <h2>✅ Banco de dados resetado com sucesso!</h2>
      <p>O banco antigo foi movido para: <code>${backupPath}</code></p>
      <p>Um banco novo e vazio foi criado.</p>
      <p><strong>⚠️ Ação necessária:</strong> Reinicie o serviço manualmente no dashboard do Render.</p>
    `);
  } catch (error) {
